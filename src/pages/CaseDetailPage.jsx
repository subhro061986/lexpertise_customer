    ...
    const fetchCase = async () => {
      try {
        const res = await fetch(
          `http://localhost:8000/documents/${uuid}`
        );
      ...
      <main className="flex justify-center px-4 py-10 bg-gray-100 min-h-screen">
        <div className="w-full max-w-3xl bg-white rounded-3xl border border-gray-200 shadow-sm p-8">
          ...
          <div className="flex gap-6 mt-8 items-center justify-center">
            ...
            <button
              onClick={() => openPdf(caseData.pdf_links?.main)}
              className="px-6 py-3 rounded-full border border-yellow-500 text-yellow-600 font-semibold"
            >
              View
            </button>
            ...
            <button
              ...
              className="px-6 py-3 rounded-full bg-yellow-500 text-white font-semibold flex items-center gap-2"
            >
              Download
          ...
          <div className="grid grid-cols-3 gap-6 text-center">
            {[
              ...
              <div
                key={index}
                onClick={() => openPdf(caseData.pdf_links?.[item.key])}
